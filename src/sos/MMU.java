...
/**
     ...
     * translate()
     * 
     * This method takes a given virtual address and translates it to a
     * physical address via a page -> frame mapping.
    ...
    private int translate(int virtAddr)
    {
        ...
        //given virtual address
        int page   = (virtAddr & m_pageMask) >> m_offsetSize;
        int frameNumber = m_RAM.read(page);
        
        if(frameNumber < 0)
        {
        	System.out.println("NEGATIVE FRAME NUMBER! " + frameNumber);
        	
        }
        
        // Offset from the base of a frame
        int offset = (virtAddr & m_offsetMask);
        
        // Calculate the base address of the frame
        int baseOfFrame = (frameNumber * m_pageSize);
        
        return (baseOfFrame + offset);
    }//translate
